...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            bool result = false;
            if (Emps.Where(e => e.Job == "Backend programmer").Count() >= 1)
                result = true;
            return result;
        }
        ...
        public static IEnumerable<object> Task10() {
            ...
            var value = new { Ename = "Brak wartości", Job = string.Empty, HireDate = (DateTime?)null }; 
            var list = new List<object>(); list.Add(value); result = Emps.Select(e => new { e.Ename, e.Job, e.HireDate }).Union(list); 
            return result;
        }
        ...
        public static IEnumerable<object> Task11()
        {
            IEnumerable<object> result = Emps.Join(Depts, e => e.Deptno, d => d.Deptno, (emp, dept) => new
            {
                name = dept.Dname,
                numOfEmployes = emp
            }).GroupBy(e => e.name).Where(gr => gr.Count()> 1).Select(gr => new { name = gr.Key, numOfemployes = gr.Count()}); 
            return result;
        }
        ...
        {
            int result = 0;
            result = arr.GroupBy(x => x).Where(gr => gr.Count() % 2 == 1).Select(gr => gr.Key).Single();
            return result;
        }
